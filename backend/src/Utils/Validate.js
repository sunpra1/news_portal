...
const userData = async data => {
    ...
    if (data.fullName) {
        if (Validator.trim(data.fullName.toString()).length == 0) {
    ...
    if (data.password) {
        if (Validator.trim(data.password.toString()).length == 0) {
...
const loginData = data => {
    ...
    if (data.email) {
        if (Validator.trim(data.email).length == 0) {
            error.email = "Email is left empty";
        } else if (!Validator.isEmail(Validator.trim(data.email))) {
            error.email = "Please provide valid email address";
        }
    } else {
...
export { userData, uniqueUserData, loginData, userUpdateData, validateCategory }